...
class DataUtils:
    ...
    def bivariate_analysis(self, app_vars, total_dl_col, total_ul_col):
        results = {}
        figs = []  # Initialize the list to store figures
        ...
        for app_var in app_vars:
            if app_var in self.df.columns:
                ...
                correlation_ul = self.df[[app_var, total_dl_col]].dropna().corr().iloc[0, 1]
                ...
                sns.regplot(x=app_var, y=total_dl_col, data=self.df, scatter_kws={'s': 10}, line_kws={'color': 'red'}, ax=ax[0])
                ax[0].set_title(f'{app_var} vs {total_dl_col}')
                ax[0].set_xlabel(app_var)
                ax[0].set_ylabel(total_dl_col)

                sns.regplot(x=app_var, y=total_ul_col, data=self.df, scatter_kws={'s': 10}, line_kws={'color': 'blue'}, ax=ax[1])
                ax[1].set_title(f'{app_var} vs {total_ul_col}')
                ax[1].set_xlabel(app_var)
                ax[1].set_ylabel(total_ul_col)
                ...
                buf.seek(0)
                figs.append(buf)  # Store the BytesIO object

                plt.close(fig)  # Close the figure to prevent overlapping issues

        # Return the DataFrame of correlations and the list of BytesIO figures
        return pd.DataFrame(results).T, figs
    def clean_data(self):
